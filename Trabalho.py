...
def desenha_semaforos(screen, semaforo, coordenadas_semaforos):
    posicao = 0
...
def liga_semaforo(screen, posicao, semaforo, coordenadas_semaforos):
    x,y = coordenadas_semaforos[posicao]
    if posicao in cima():
        screen.blit(semaforo.direcao.cima, (x, y))
    if posicao in esquerda():
...
def desenha_carro(screen, carro, estrada, direcao):
    tamanho_preto_ = (largura - (num_linhas*tamanho_espessura)) // num_linhas
    ...
    if direcao == "cima":
        x = tamanho_espessura//10 + estrada*(tamanho_espessura+tamanho_preto_)
        y = altura - alt_carro
        screen.blit(carro.direcao.cima, (x, y))
    if direcao == "esquerda":
        x = largura - alt_carro
        y = tamanho_espessura//2 + tamanho_preto_ + estrada*(tamanho_espessura+tamanho_preto_)
        screen.blit(carro.direcao.esquerda, (x, y))
    if direcao == "direita":
        x = 0
        y = tamanho_espessura//2 + tamanho_preto3 + estrada*(tamanho_espessura+tamanho_preto_)
        screen.blit(carro.direcao.direita, (x, y))
    if direcao == "baixo":
        x = tamanho_espessura//10 + tamanho_preto2 + estrada*(tamanho_espessura+tamanho_preto_)
        y = 0
        screen.blit(carro.direcao.baixo, (x, y))
...
async def main():
    ...
    while running:
        ...
        desenha_semaforos(screen, semaforo_cinza, coordenadas_semaforos)
        liga_semaforo(screen, 10, semaforo_verde, coordenadas_semaforos)
        liga_semaforo(screen, 7, semaforo_verde, coordenadas_semaforos)
        liga_semaforo(screen, 5, semaforo_vermelho, coordenadas_semaforos)

        desenha_carro(screen, carro_vermelho, 0, "direita")
        desenha_carro(screen, carro_preto, 1, "esquerda")
        desenha_carro(screen, carro_azul, 0, "cima")
        desenha_carro(screen, carro_vermelho, 2, "baixo")
